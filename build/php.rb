class Php < Package
  ...
  def get_build_string
    ...
    # Built-in extensions we must explicitly turn on because of --disable-all
    parts += [
      "--enable-rpath",
  ...
  def rake
    namespace to_sym do
      ...
      task :compile => :configure do
        if compile then
          notice "PHP successfully compiled. Now type `rake php:install` to install."
        end
      end
    ...
    task to_sym => "#{underscored}:compile"
  end
end
